...
export function ManualApp({
  ...
  const runWithInlineAuth = useCallback(async (action) => {
    try {
      ...
      setActivePanel({
        ...
        text: 'Resource Optimiser authentication required. Starting terminal auth...'
      });
      await workflows.loginResourceOptimiser();
      return action();
    }
  }, [workflows]);
  ...
  }, [controller, cwd, exit, prompts, setCommandPanel, workflows]);
  ...
  const startNewLogworkWizard = useCallback(async () => {
    ...
    try {
      const weekly = await runWithInlineAuth(() => workflows.queryLogwork({
        period: 'this_week',
        cwd,
  ...
  }, [cwd, runWithInlineAuth, workflows]);
  ...
  const selectWizardDate = useCallback(async (dateOption) => {
    ...
    try {
      const projectsResult = await runWithInlineAuth(() => workflows.listLogworkProjects({ cwd }));
  ...
  }, [cwd, runWithInlineAuth, workflows, wizard]);
  ...
  }, [cwd, previewDraft, runWithInlineAuth, workflows]);
...
export function StatusBar({ preview, dryRun, wizard }) {
  const previewText = preview
    ? `preview: ${preview.status} · ${preview.entries?.length || 0} entries`
    : 'preview: none';
  const mode = wizard?.step ? `mode: ${wizard.step.replace('_', '-')}` : 'mode: command';
  const dryRunText = dryRun ? ' · DRY RUN' : '';
  const hint = wizard?.step === 'edit_tasks' ? 'Type / for task commands' : 'Type / for commands';
  ...
  h(Text, { color: wizard?.step ? 'yellow' : 'gray' }, `${mode} · ${previewText}${dryRunText} · ${hint}`));
}
