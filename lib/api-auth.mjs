...
export async function authenticateWithApiSession({
  ...
  const session = createHttpSession({
    fetchImpl,
    allowedHosts: protocolContext.allowedHosts,
...
function createAuthProtocolContext({
  ...
  return {
    allowedHosts: [
      hostFromUrl(apiBase),
      hostFromUrl(keycloakAuthUrl),
      hostFromUrl(keycloakTokenUrl),
      hostFromUrl(keycloakRedirectUri)
    ].filter(Boolean),
    keycloakHost: hostFromUrl(keycloakAuthUrl),
    redirectUri: keycloakRedirectUri
  };
}

function assertAllowedFormAction(form, context) {
  const actionHost = hostFromUrl(form.action);
  if (!actionHost || actionHost !== context.keycloakHost) {
    throw safeApiAuthError(`Blocked Keycloak form submission to unexpected host: ${actionHost || 'unknown-host'}.`);
  }
}

function assertAllowedRedirect(rawUrl, { allowedHosts, authorizationRedirectUri }) {
  const host = hostFromUrl(rawUrl);
  if (!host || !allowedHosts.has(host)) {
    throw safeApiAuthError(`Blocked auth redirect to unexpected host: ${host || 'unknown-host'}.`);
  }
