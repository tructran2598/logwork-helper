...
export async function generateDiagnosticsReport({
  ...
  const configSnapshot = await safeCollect('config snapshot', () => collectConfigSnapshot({ cwd }));
...
async function collectConfigSnapshot({ cwd }) {
  const config = await loadLocalConfig(cwd);
  return {
    userConfigPath: configPath(cwd, 'user'),
    projectConfigPath: configPath(cwd, 'project'),
    mappingCount: config.projectMappings.length,
    mappings: config.projectMappings.map((mapping) => ({
