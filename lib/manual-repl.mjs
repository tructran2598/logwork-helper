...
export async function executeManualCommand(command, session, context) {
  ...
  if (command.type === 'apply') {
    ...
    if (session.lastPreview.errors?.length) {
      print('Cannot apply preview with parse errors. Run /logwork again after fixing the text.');
      return { exit: false };
    }

    if (session.lastPreview.unresolvedEntries?.length) {
      print(`Cannot apply preview with ${session.lastPreview.unresolvedEntries.length} unresolved entries. Use /projects and /map, then /logwork again.`);
      return { exit: false };
    }

    const totalHours = totalPreviewHours(session.lastPreview);
    const entryCount = session.lastPreview.entries?.length || 0;
    const confirmed = await prompts.confirm(`Apply ${entryCount} logwork entries totaling ${formatHours(totalHours)}h?`);
    if (!confirmed) {
    ...
    if (session.lastPreview.unbookedEntries?.length) {
      allowUnbooked = await prompts.confirm(`This preview contains ${session.lastPreview.unbookedEntries.length} unbooked entries. Submit them anyway?`);
      if (!allowUnbooked) {
...
export function formatProjects(result, weekly = null, projectFilter = undefined) {
  ...
  const otherProjects = projects.filter((project) => !chartedProjectKeys.has(projectKey(project)));
...
export function formatWeeklyProjectCharts(projects, weekly, projectFilter, chartedProjectKeys = new Set()) {
  ...
  for (const weeklyProject of weeklyProjects) {
    const membership = projects.find((project) => sameProject(project, weeklyProject)) || weeklyProject;
    const key = projectKey(membership);
...
function projectDays(days, project) {
  return days
    .map((day) => {
      const matching = (day.projects || []).find((candidate) => sameProject(candidate, project));
      if (!matching) {
        return null;
      }
      return {
...
function projectMatchesFilter(project, projectFilter) {
  const filter = normalize(projectFilter);
  return String(project.projectMemberId) === String(projectFilter) ||
    String(project.projectId) === String(projectFilter) ||
    normalize(project.projectName).includes(filter);
}
...
function sameProject(left, right) {
  ...
  return normalize(left.projectName) === normalize(right.projectName);
}

function projectKey(project) {
  return String(project.projectMemberId ?? project.projectId ?? normalize(project.projectName));
}
...
function totalPreviewHours(preview) {
