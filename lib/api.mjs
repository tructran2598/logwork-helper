...
function readProjectName(row, fallbackId) {
  ...
  return fallbackId !== undefined && fallbackId !== null && String(fallbackId).trim() !== ''
    ? projectFallbackName(fallbackId)
    : 'Unknown project';
}
...
function readLogEntries(row) {
  ...
  return rawEntries
    ...
    .map((entry) => ({
      ...
      projectName: hasProjectName(entry) ? readProjectName(entry, readProjectId(entry)) : readProjectName(row, readProjectId(row) ?? readProjectMemberId(row)),
